...
class FacultyLoadingAjax extends Controller
{
    ...
    function accept_load(){
        if(Request::ajax()){
           ...
           $notification->content = "Instructor ".strtoupper($user->lastname).', '.strtoupper($user->name).' accepted the faculty load suggested by the Coordinator.';
           $notification->save();
           
           $content = "Instructor ".strtoupper($user->lastname).', '.strtoupper($user->name).' accepted the faculty load suggested by the Coordinator.';
